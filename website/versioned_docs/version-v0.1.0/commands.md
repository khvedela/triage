---
title: Commands
description: Reference for triage commands, subcommands, and high-signal global flags.
slug: /commands
---
...
<InfoGrid
  ...
  items={[
    ...
    },
    {
      ...
      body: "Use rule and config commands when you need to understand why triage behaves a certain way or how it is currently configured."
    },
    {
      label: "Publish",
      title: "Report output",
    ...
    },
    {
      ...
      body: "Use completion scripts and JSON output when you want triage to feel native in shells and automation pipelines."
    }
  ]} />
...
<CommandBlock
  ...
  command="triage report namespace prod > triage-report.md"
  caption="`triage report cluster` is reserved for a future release and currently returns a not-yet-implemented error." />
...
| `--confidence-min` | Useful when you want triage to report only higher-certainty diagnoses. |
...
| `--config` | Points triage at a specific config file instead of the default path. |

Kubernetes client flags such as `--context`, `--kubeconfig`, `--cluster`, and `--user` are also available because triage uses the standard Kubernetes CLI runtime stack.
